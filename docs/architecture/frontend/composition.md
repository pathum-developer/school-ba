...
| Content | `src/data/school.ts` | Language-independent records and their shapes |
| Translation | `src/i18n/`, `src/locales/` | Every user-visible string, both layers |
| Utilities | `src/hooks/`, `src/lib/` | Browser-facing hooks, the `cn` class merger |
...
```
main → App → home/ → ui/ → lib/
                ↘ data/, i18n/, hooks/, lib/
```

Concretely:

- `ui/` never imports from `home/` or `data/`. A primitive that knows about courses is no
  longer a primitive.
- `data/` imports nothing from the application at all — it is plain records and one
  formatting helper.
- `i18n/` may read `data/` (`useMoney` calls `formatAmount`), never the reverse.
...
them positions its own heading.
