...
> **Status: partly built.** The shell, the permission-filtered navigation and the route
> guards exist and are mounted at `/app` under React Router. Authentication does not — the
> session comes from `DemoSessionProvider`, a stand-in with an on-screen role picker. Who
> the users are and what they may do is summarized in
...
**A router is required.** ~~None is installed.~~ **Done.** React Router is installed and
`App.tsx` is a `RouterProvider`; the home page lifted into `pages/HomePage.tsx` as a public
route and the shell is the second route tree, under `/app`. See
[`routes.tsx`](../../../../school-ui/src/routes.tsx).
...
```
src/
├── routes.tsx             # the route table — public tree + /app tree
├── pages/
│   ├── HomePage.tsx       # the public site
│   └── NotFoundPage.tsx   # catch-all, outside both trees
├── app/
│   ├── AppShell.tsx       # the /app layout route, and where /app itself lands
...
│   ├── RequirePermission.tsx  # route guard + the 403 body
│   ├── DemoSessionProvider.tsx # stand-in for auth — delete when sign-in is real
...
`useSession` throws outside a signed-in route. The shell is mounted at `/app`, and
[`DemoSessionProvider`](../../../../school-ui/src/app/DemoSessionProvider.tsx) fills the context with
one of the demo sessions plus an on-screen role picker — a stand-in for authentication, not
authentication. The route guards read `SessionContext` rather than that component, so
replacing it with the real provider leaves the route tree untouched.

Layer 2 of the three below is [`RequirePermission`](../../../../school-ui/src/app/RequirePermission.tsx),
applied to every `/app` child route from the same permission that decides whether its
