# Learner Package Instructions

This package owns learner-related domain code.

## Package Scope

- Keep all learner controllers, services, repositories, entities, DTOs, mappers, and enums under `learner`.
- Model learners as school-owned records. Do not introduce a global learner or a shared cross-school profile abstraction unless the architecture documents are updated first.
- Keep learner enrollment, current branch assignment, and same-school branch transfer workflows under `learner`.
- Do not place learner-specific business rules in `user`, `auth`, `common`, or `config`.
...
- Put learner-specific enum types under `learner.enums`.
